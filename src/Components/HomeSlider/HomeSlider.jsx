            ...
            loop={true}>
                ...
                slideProduct.map((product)=> <SwiperSlide key={product.productId}>
                  <div className='w-full flex justify-around items-center gap-2 lg:gap-100 p-2 lg:p-10 bg-gradient-to-r from-green-300 via-green-600 to-teal-700 rounded-xl relative '>
                    <img className='w-20 h-20 sm:w-150 sm:h-120 rounded-xl' src={product.pictureURL} alt="product-photo" />
                 <div className='mr-0 pr-0 lg:mr-10 lg:pr-20'>
                     <h1 className='text-[14px] lg:text-3xl font-bold text-[#001931] text-center'>
                    {product.productName}
                  </h1>
                    <p className='text-[12px] lg:text-xl py-1 lg:py-3 text-center font-medium'>Price $ {product.price}</p>
                    <p className='text-[10px] lg:text-[18px] mb-5 text-[#d0d6db]'>{product.description}</p>
                 </div>
